...
String? countryFlag(String? countryName) {
  if (countryName == null) {
    return 'https://flagcdn.com/h80/in.png';
  }
...
String? timerformat(
  ...
  int totalMilliseconds = (var1 ?? 0) + (var2 ?? 0) + (var3 ?? 0) + (var4 ?? 0);
...
String encryptData(String plainText) {
  String keyStr = "predefined_secret_key"; // Predefined key
  int desiredLength =
      plainText.length + 13; // Fixed length based on the example provided
  List<int> keyBytes = keyStr.codeUnits;
  List<int> plainBytes = utf8.encode(plainText);
  List<int> paddedPlainBytes = List<int>.from(plainBytes)
    ..addAll(List<int>.filled(desiredLength - plainBytes.length, 0));
  List<int> encryptedBytes = List<int>.filled(desiredLength, 0);

  for (int i = 0; i < paddedPlainBytes.length; i++) {
    encryptedBytes[i] =
        (paddedPlainBytes[i] + keyBytes[i % keyBytes.length]) % 256;
  }
...
String decryptData(String encryptedText) {
  ...
  List<int> decryptedBytes = List<int>.filled(encryptedBytes.length, 0);
  ...
  // Remove padding
  List<int> unpaddedDecryptedBytes =
      decryptedBytes.takeWhile((value) => value != 0).toList();

  return utf8.decode(unpaddedDecryptedBytes);
}

List<String>? decryptDataList(List<String> encryptedList) {
  String keyStr = "predefined_secret_key"; // Predefined key
  List<String> decryptedList = [];

  for (String encryptedText in encryptedList) {
    List<int> keyBytes = keyStr.codeUnits;
    List<int> encryptedBytes = base64.decode(encryptedText);
    List<int> decryptedBytes = List<int>.filled(encryptedBytes.length, 0);
    ...
    decryptedList.add(utf8.decode(unpaddedDecryptedBytes));
  }
